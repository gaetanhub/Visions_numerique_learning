...
function useExamMode(questions: QuizItem[]): {
  running: boolean;
  timeLeft: number;
  index: number;
  score: number;
  ...
  function answer(value: number): void {
    const correct = value === questions[index].answerIndex;
  ...
  return { running, timeLeft, index, score, answered, lastResult, start, answer };
}
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        localStorage.removeItem(`vn-progress-${module.id}`);
      }
    }
    ...
    <article className="card" id={module.id}>
      ...
      <section>
        ...
        <div className="row wrap">
          ...
          <span className="timer">Score: {exam.score}/{exam.answered}</span>
        </div>
        {exam.running && (
        ...
        {exam.lastResult !== null && <p className={exam.lastResult ? 'ok' : 'ko'}>{exam.lastResult ? 'Réponse correcte.' : 'Réponse incorrecte.'}</p>}
        {!exam.running && exam.answered > 0 && <p className="source">Résultat examen: {exam.score}/{exam.answered}</p>}
      </section>
